...
class WindowClass(QMainWindow, form_class):
    ...
    def book_lookup(self):
        ...
        for res in search_result:
            ...
            btn.clicked.connect(lambda: self.rental_click(books[res].title))  # 버튼 시그널 추가
            self.table_search.setItem(col, 8, QTableWidgetItem(status_view(books[res].return_date)))
            col += 1

        # btn.clicked.connect(self.rental_click(books[res]))
    ...
    def rental_click(self):
        btn = self.sender()
        # btn.pos = btn의 현재 위치를 튜플 좌표로 반환하여 item에 할당
        item = self.table_search.indexAt(btn.pos())
        # item을 기준으로 도서명을 가져옴
        title = self.table_search.item(item.row(), 1).text()
        print('대여 책 제목',title)

        if self.now_user is None: # 비회원
            QMessageBox.information(self, '', '로그인 먼저 해주세요')
        else:   # 회원
            book_cnt = 5
            if self.now_user.book1 is None:
                book_cnt -= 1
            if self.now_user.book2 is None:
                book_cnt -= 1
            if self.now_user.book3 is None:
                book_cnt -= 1
            if self.now_user.book4 is None:
                book_cnt -= 1
            if self.now_user.book5 is None:
                book_cnt -= 1

            date = datetime.date.today()
            buttonReply = QMessageBox.information(self, '알림', '대여 가능 권수: '+str(book_cnt)+'\n대여일자: '+str(date)+
                                                  '\n [' + title + '] \n 대여하시겠습니까?', QMessageBox.No | QMessageBox.Yes,
                                              QMessageBox.Yes)
            # 기본 선택 = Yes
